    ...
    <div class="form-container">
        ...
        <form class="form" action="{{ route('ouvrages.update', $ouvrage->id_ouvrage) }}" method="POST">
            ...
            <div class="form-group">
                ...
                <input type="text" id="code_isbn" name="code_isbn" value="{{ old('code_isbn', $ouvrage->code_isbn) }}">
                @error('code_isbn')
                    <span class="text-danger">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <select id="id_editeur" name="id_editeur" required>
                    ...
                    @foreach ($editeurs as $editeur)
                        <option value="{{ $editeur->id_editeur }}" {{ old('id_editeur', $ouvrage->id_editeur) == $editeur->id_editeur ? 'selected' : '' }}>
                            {{ $editeur->libelle }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select id="auteurs" name="auteurs[]" multiple required>
                    @foreach ($auteurs as $auteur)
                        <option value="{{ $auteur->id_auteur }}" {{ in_array($auteur->id_auteur, old('auteurs', $ouvrage->auteurs->pluck('id_auteur')->toArray())) ? 'selected' : '' }}>
                    ...
                    @foreach ($genres as $genre)
                        <label>
                            <input type="checkbox" name="genres[]" value="{{ $genre->id_genre }}"
                                {{ in_array($genre->id_genre, old('genres', $ouvrage->genres->pluck('id_genre')->toArray())) ? 'checked' : '' }}>
                            {{ $genre->libelle }}
                        </label>
                    @endforeach
                </div>
